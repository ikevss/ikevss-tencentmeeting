...
| 即将执行 | 必须 Read |
|---------|----------|
| 阶段 2 邮件邀请 | `scripts/email-invite-spec.md` |
| 阶段 3 会后复盘 Phase A 起 | `scripts/insight-rendering.md` + `scripts/insight-examples.md` |
...
> **加载契约**：首次发送前，必须 Read `scripts/email-invite-spec.md`（含 ICS 模板 + HTML/CSS + 两阶段确认）。
...
> **加载契约**：进入 Phase A 前，必须 Read `scripts/insight-rendering.md` + `scripts/insight-examples.md`。
