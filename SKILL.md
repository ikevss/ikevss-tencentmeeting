...
> "✅ 会议已创建。需要给参会人发送邮件邀请吗？我可以生成精美的响应式邀请函（含会议信息卡片 + 一键入会按钮 + 日历日程附件），桌面端和手机端都能正常浏览。邮件会附带 .ics 日历文件，收件人双击即可将会议添加到自己的日历，会议开始前自动提醒。"
...
📌 日程说明：当前 agently-cli 不支持 iMIP（text/calendar MIME part），
   无法在邮件正文中直接渲染"接受/拒绝/暂定"按钮。
   .ics 附件为当前最优方案：收件人双击文件即可加入日历，
   Outlook / Apple Mail / Gmail 全平台兼容，会议开始前自动提醒。
   待 agently-cli 支持 raw MIME 后升级为 iMIP 内嵌日程。
```
