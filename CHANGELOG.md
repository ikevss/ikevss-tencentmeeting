...
### 🐛 修复
- **SKILL.md frontmatter**: `description: >` 多行折叠符改为单行 YAML——SuperIR 逐行解析时只拿到 `>` 字符，后续缩进行因不含冒号被跳过，导致技能卡片描述乱码/空白
- **版本号统一**: VERSION / package.json 从 2.2.5 同步到 2.2.6（此前仅 SKILL.md 升版）
